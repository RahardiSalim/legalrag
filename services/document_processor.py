...
from utils.text_splitters import HierarchicalLegalSplitter
...
class DocumentProcessor(DocumentProcessorInterface):
    ...
    def _detect_document_type(self, content: str) -> str:
        ...
        legal_score = sum(1 for indicator in settings.document.legal_indicators 
                         if indicator in content_lower)
        
        return 'legal' if legal_score >= settings.document.legal_detection_threshold else 'general'
    ...
    def _load_single_document(self, file_path: str) -> List[Document]:
        ...
        if file_path.stat().st_size > settings.document.max_document_size:
    ...
    def _process_chunks(self, chunks: List[Document]) -> List[Document]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if not content or len(content.strip()) < settings.document.min_chunk_length:
                continue
